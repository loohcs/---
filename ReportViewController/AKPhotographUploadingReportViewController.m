...
- (IBAction)chooseDrugstore:(id)sender//选择药店
{
...
- (IBAction)photoLeixing:(id)sender//照片类型
{
    [self touchesBegan:nil withEvent:nil];
...
- (IBAction)photoZhonglei:(id)sender//照片种类
{
    [self touchesBegan:nil withEvent:nil];
    UIButton *btn = (UIButton*)sender;
    buttonInt = btn.tag;
...
{
    NSString *oneText = [NSString stringWithFormat:@"%@",[aDic objectForKey:@"MNo"]];
    self.textField1.text = oneText;
    NSString *twoText = [NSString stringWithFormat:@"%@",[aDic objectForKey:@"MName"]];
