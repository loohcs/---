...
@interface AKDailyViewController ()
...
@implementation AKDailyViewController
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"我的日报" andtextAlignment:UITextAlignmentCenter andFont:18];
...
-(void)ransmitRequest//发送请求
{
//    http://skyeyes.ip165.com/yyt/port/dayreport.aspx?cid=444&mobile=13406957927&pwd=1&action=list&pagenum=&id=&employno=&employ=&title=&content=&stime=&etime=&rcontent=
    mum.hidden = NO;
    ...
    [currentRequestContent currentRequestWithURL:_YGRIBAO_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
//    NSLog(@"%@",aContentDic);
    ...
    if ([flagStr isEqualToString:@"success"]) {
        NSArray *listArray = [aContentDic objectForKey:@"DaysWork"];
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView dailyCellViewWithArray:listArray]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    AKDailyDetailsViewController *dailyDetailsVC = [[AKDailyDetailsViewController alloc]init];
    dailyDetailsVC.detailsId = idStr;
    [self.navigationController pushViewController:dailyDetailsVC animated:NO];
    [dailyDetailsVC release];
}
