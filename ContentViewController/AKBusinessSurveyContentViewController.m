//
//  AKBusinessSurveyContentViewController.m
//  医药通
//
...
@interface AKBusinessSurveyContentViewController ()
...
@implementation AKBusinessSurveyContentViewController
...
- (void)addQueryView
{
    ...
    UILabel *topLabel = [AKFastLabel labelWithFrame:CGRectMake(50, 0, 280, 44) andText:@"商务调查查询" andtextAlignment:UITextAlignmentLeft andFont:22];
    topLabel.textColor = [UIColor whiteColor];
    UILabel *queryLabel1 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 50, _SIX_WIDTH_, _LINE_HEIGHT_) andText:@"商务区域编码：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_];
    queryLabel1.textColor = [UIColor whiteColor];
    queryTextField1 = [AKFastTextField textFieldWithFrame:CGRectMake(_SIX_WIDTH_+5, 50, 133, 30) andBackgroundColor:[UIColor whiteColor] andBorderStyle:UITextBorderStyleBezel andKeyboardType:UIKeyboardTypeASCIICapable andFont:15.f];
    queryTextField1.tag = 401;
    queryTextField1.delegate = self;
    queryTextField1.text = @"";
    UILabel *queryLabel2 = [AKFastLabel labelWithFrame:CGRectMake(_START_INTERVAL_, 65+_LINE_HEIGHT_, _SIX_WIDTH_, _LINE_HEIGHT_) andText:@"商务区域名称：" andtextAlignment:UITextAlignmentLeft andFont:_QUERY_LABEL_FONT_]; 
    queryLabel2.textColor = [UIColor whiteColor];
    queryTextField2 = [AKFastTextField textFieldWithFrame:CGRectMake(_SIX_WIDTH_+5, 90, 133, 30) andBackgroundColor:[UIColor whiteColor] andBorderStyle:UITextBorderStyleBezel andKeyboardType:UIKeyboardTypeDefault andFont:15.f];
...
- (void)viewDidLoad
{
    ...
    UILabel *topItemLabel = [AKFastLabel labelWithFrame:CGRectMake(0, 0, 180, 30) andText:@"商务调查" andtextAlignment:UITextAlignmentCenter andFont:18];
...
-(void)ransmitRequest//发送请求
{
    ...
    [currentRequestContent currentRequestWithURL:_SWDIAOCHA_INTERFACE_ andParameter:parameterDic];
}
-(void)ransmitRequestWithDic:(NSDictionary*)aDic//发送请求
{
    mum.hidden = NO;
...
{
//    http://skyeyes.ip165.com/yyt/port/bsearch.aspx?cid=008&mobile=18953553903&pwd=123&action=list&pagenum=&id=&areano=&areaname=&mno=&mname=&linkman=&mob=
    [self touchesBegan:nil withEvent:nil];
...
//    NSLog(@"%@",aContentDic);
    ...
    if ([flagStr isEqualToString:@"success"]) {
        NSArray *listArray = [aContentDic objectForKey:@"mequipment"];
        if (listArray.count) {
            [cellViewArray setArray:[AKFastCallView businessSurveyCellViewWithArray:listArray]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    surveyDetailsVC.surveyURL = _SWDIAOCHA_INTERFACE_;
